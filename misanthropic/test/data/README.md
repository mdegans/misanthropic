...
| `server_tools/pause_turn.sse.stream.jsonl` | live (`web_fetch`, Haiku 4.5, `capture.sh`, 2026-06-10) | captured (12 forced-sequential fetches of tiny httpbin pages hit the server-loop iteration cap: 11 calls issued, 10 results, then `stop_reason: pause_turn`. **The turn pauses mid-call.** Tiny pages keep the echo under tier-1 TPM — a web_search pause echo was ~84k tokens and unresendable; request in `requests/pause_turn.json`) |
| `server_tools/pause_turn_resume.sse.stream.jsonl` | live (`web_fetch`, Haiku 4.5, `capture.sh`, 2026-06-10) | captured (the continuation: echo the assembled paused turn back, same tools, no new user message. `message_start` is **empty** — a fresh adjacent assistant turn, unlike the pre-populated PTC resume — it delivers the in-flight 11th result, runs the 12th fetch, ends `end_turn`) |
