...
| `prompt_caching` | **Prompt caching** with `CachedPrompt` — cache the crate's own docs as a frozen prefix, then watch `cache write` flip to `cache read` across turns. | `client` |
